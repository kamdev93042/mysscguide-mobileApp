...
export default function MockPracticeScreen() {
  ...
  const pausedTestsStorageKey = withUserScope(PAUSED_TESTS_STORAGE_KEY, storageScope);
  ...
  const persistSubmissionResult = async (result: any) => {
    try {
      ...
      await AsyncStorage.setItem(resultHistoryStorageKey, JSON.stringify(next));
    } catch (error) {
  ...
  const persistPausedPayload = async (pausedPayload: any) => {
    try {
      ...
      await AsyncStorage.setItem(pausedTestsStorageKey, JSON.stringify(next));
    } catch (error) {
